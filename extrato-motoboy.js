...
window.montarExtratoMotorista=async function(){
  ...
  const q=query(collection(db,"movimentacoes"), where("usuarioId","==",auth.currentUser.uid), where("tipo","==","CREDITO"), orderBy("data","desc"));
